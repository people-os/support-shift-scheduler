...
export async function getJWTAuthClient() {
	if (!process.env.GAPI_SERVICE_ACCOUNT_JWT) {
		throw new Error(
			'Environment variable GAPI_SERVICE_ACCOUNT_JWT is not set.',
		);
	}
	let jwt: any;
	try {
		...
		const jwtContent = await fs.readFile(
			process.env.GAPI_SERVICE_ACCOUNT_JWT,
			'utf8',
		);
		jwt = JSON.parse(jwtContent); // Parse the content of the file
	}
