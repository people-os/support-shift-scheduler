...
def setup_var_dataframes_onboarding(agent_categories, config):
    ...
    mentors_indices = pd.MultiIndex.from_product(
        [[d for d in range(config["num_days"])], agent_categories["onboarding"]], names=("day", "agent")
    )
    ...
    var_onboarding["h"] = pd.DataFrame(
        data=None, index=agent_categories["onboarding"], columns=["total_week_slots"]
    )

    # dh:
    dh_multi_index_on = pd.MultiIndex.from_product(
        [[d for d in range(config["num_days"])], [h for h in agent_categories["onboarding"]]],
        names=("day", "handle"),
    )
...
def fill_var_dataframes_onboarding(model, custom_domains, var_onboarding, df_agents, agent_categories, config):
    ...
    for d in range(config["num_days"]):
        for h in agent_categories["onboarding"]:
            if h in agent_categories["unavailable"][d]:
                var_onboarding["dh"].loc[(d, h), "shift_start"] = model.NewIntVar(
                    8, 8, f"shift_start_{d}_{h}"
                )
                var_onboarding["dh"].loc[(d, h), "shift_end"] = model.NewIntVar(
                    8, 8, f"shift_end_{d}_{h}"
                )
                var_onboarding["dh"].loc[(d, h), "shift_duration"] = model.NewIntVar(
                    0, 0, f"shift_duration_{d}_{h}"
                )

            else:
                var_onboarding["dh"].loc[(d, h), "shift_start"] = model.NewIntVarFromDomain(
                    custom_domains["prefs"].loc[(d, h)], f"shift_start_{d}_{h}"
                )
                var_onboarding["dh"].loc[(d, h), "shift_end"] = model.NewIntVarFromDomain(
            ...
            var_onboarding["dh"].loc[(d, h), "interval"] = model.NewIntervalVar(
    ...
    for d in range(config["num_days"]):
        for h in agent_categories["onboarding"]:
            for s in range(config["start_slot"], config["end_slot"]):
                ...
                var_onboarding["dhs"].loc[(d, h, s), "is_slot_cost"] = model.NewBoolVar(
    ...
    for d in range(config["num_days"]):
        for h in agent_categories["onboarding"]:
            if not (h in agent_categories["unavailable"][d]):
                model.AddBoolOr(var_onboarding["dh"].loc[(d, h), "is_in_pref_range"])
                # TODO: I think .OnlyEnforceIf(var_onboarding["dh"].loc[(d, h), "is_agent_on"]) should be added above.
                for (j, sec) in enumerate(df_agents.loc[h, "slot_ranges"][d]):
                    model.Add(
                        var_onboarding["dh"].loc[(d, h), "shift_start"] >= sec[0]
                    ).OnlyEnforceIf(var_onboarding["dh"].loc[(d, h), "is_in_pref_range"][j])
                    model.Add(
                        var_onboarding["dh"].loc[(d, h), "shift_start"]
                        + var_onboarding["dh"].loc[(d, h), "shift_duration"]
                        <= sec[1]
...
def constraint_setup_onboarding_hours(model, var_onboarding, agent_categories, config):
    ...
    for d in range(config["num_days"]):
        for h in agent_categories["onboarding"]:
            ...
            ).OnlyEnforceIf(var_onboarding["dh"].loc[(d, h), "is_agent_on"].Not())
    ...
    for h in agent_categories["onboarding"]:
        ...
        model.Add(var_onboarding["h"].loc[h, "total_week_slots"] == onboarding_weekly_slots)
    return model


def constraint_avoid_onboarding_before_Monday_1400(model, var_onboarding, agent_categories):
    ...
    for h in agent_categories["onboarding"]:
        model.Add(var_onboarding["dh"].loc[(0, h), "shift_start"] >= 28).OnlyEnforceIf(
            var_onboarding["dh"].loc[(0, h), "is_agent_on"]
        )
    return model
...
def constraint_avoid_simultaneous_onboarding(model, var_onboarding, config):
    ...
    if len(var_onboarding["dh"]) > 0:
        for d in range(config["num_days"]):
            model.AddNoOverlap(var_onboarding["dh"].loc[d, "interval"].values.tolist())

    return model


def constraint_configure_mentoring(model, var_veterans, var_onboarding, agent_categories, config):
    ...
    for d in range(config["num_days"]):
        for m in var_onboarding["mentors"].columns:
            model.Add(sum(var_onboarding["mentors"].loc[(d,), m].values.tolist()) < 2)
...
def cost_hours_onboarding(model, var_onboarding, coefficients, df_agents, agent_categories, config):
        ...
        for h in agent_categories["onboarding"]:
            for (s_count, s_cost) in enumerate(
                ...
                model.Add(
                    ...
                    var_onboarding["dhs"].loc[(d, h, s), "is_start_smaller_equal_hour"]
                )
                model.Add(
                    var_onboarding["dh"].loc[(d, h), "shift_start"] > s
                ).OnlyEnforceIf(
                ...
                model.Add(var_onboarding["dh"].loc[(d, h), "shift_end"] > s).OnlyEnforceIf(
                    var_onboarding["dhs"].loc[(d, h, s), "is_end_greater_than_hour"]
                )
                model.Add(var_onboarding["dh"].loc[(d, h), "shift_end"] <= s).OnlyEnforceIf(
                    var_onboarding["dhs"].loc[(d, h, s), "is_end_greater_than_hour"].Not()
                )
                ...
                ).OnlyEnforceIf(var_onboarding["dhs"].loc[(d, h, s), "is_slot_cost"])
                ...
                ).OnlyEnforceIf(var_onboarding["dhs"].loc[(d, h, s), "is_slot_cost"].Not())
...
def extend_model_onboarding(model, var_veterans, custom_domains, full_cost_list, coefficients, df_agents, agent_categories, config):
    # Configure model variables:
    var_onboarding = setup_var_dataframes_onboarding(agent_categories, config)
    [model, var_onboarding] = fill_var_dataframes_onboarding(model, custom_domains, var_onboarding, df_agents, agent_categories, config)
    # Add constraints:
    model = constraint_honour_agent_availability_onboarding(model, var_onboarding, df_agents, agent_categories, config)
    model = constraint_setup_onboarding_hours(model, var_onboarding, agent_categories, config)
    model = constraint_avoid_onboarding_before_Monday_1400(model, var_onboarding, agent_categories)
    model = constraint_avoid_simultaneous_onboarding(model, var_onboarding, config)
    model = constraint_configure_mentoring(model, var_veterans, var_onboarding, agent_categories, config)
    # Add cost:
    model = cost_hours_onboarding(model, var_onboarding, coefficients, df_agents, agent_categories, config)
    
    # Extend list of cost terms:
    full_cost_list = full_cost_list + var_onboarding["dhs"]["slot_cost"].values.tolist()
